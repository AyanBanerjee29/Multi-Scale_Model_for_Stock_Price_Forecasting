...
def rank_tensor(x):
    """
    Return the ranks of elements in a tensor along the first dimension (dim=0).
    
    Args:
        x (torch.Tensor): Input tensor (e.g., shape [564, 7, 7]).
    
    Returns:
        torch.Tensor: Ranks of the input tensor elements along dim=0.
    """
    
    # 1. Get sorted indices ALONG THE FIRST DIMENSION (dim=0)
    sorted_indices = torch.argsort(x, dim=0)
    
    # 2. Create an empty tensor to hold the ranks, matching input shape
    ranks = torch.zeros_like(x, dtype=torch.float)
    
    # 3. Create the 1D rank tensor (e.g., [1., 2., ..., 564.])
    value_tensor = torch.arange(1, len(x) + 1, device=x.device).float()
    
    # 4. Reshape it to be broadcastable (e.g., [564, 1, 1])
    value_tensor = value_tensor.view(-1, *[1] * (x.dim() - 1))
    
    # 5. Expand it to match the full shape of the input (e.g., [564, 7, 7])
    value_tensor = value_tensor.expand_as(x)
    
    # 6. Use scatter_ to place the ranks (values) into the positions
    # specified by sorted_indices along dimension 0.
    ranks = ranks.scatter(0, sorted_indices, value_tensor)
